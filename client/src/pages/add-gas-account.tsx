...
export default function AddGasAccount() {
  ...
  const onSubmit = async (data: AddAccountForm) => {
    ...
    setTimeout(() => {
      toast({
        ...
        description: `${data.accountName} gas account has been ${isEditMode ? 'updated' : 'added'}`,
      });
      navigate(isEditMode && accountId ? `/gas-bill/account/${accountId}` : "/gas-bill");
    }, 1500);
  };
          ...
          <div className="max-w-md mx-auto text-center">
            <div className="border border-white/20 bg-white/5 p-12">
              ...
              <p className="text-white/60 mb-6">
                Your gas account has been {isEditMode ? 'updated' : 'added'} successfully
              </p>
              <p className="text-sm text-white/40">
                Redirecting to {isEditMode ? 'account details' : 'gas bills'}...
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="pt-20 pb-32 px-4 w-full max-w-screen-lg mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-12 h-12 border border-white/20 flex items-center justify-center">
            <Flame className="h-6 w-6 text-white/60" />
          </div>
          <div>
            <h2 className="text-lg font-light tracking-wide text-white">Gas Account Details</h2>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-[10px] text-white/60 uppercase tracking-widest">Gas Provider</FormLabel>
