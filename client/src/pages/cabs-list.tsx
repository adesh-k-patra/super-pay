...
import { Car, Search, Calendar, MapPin, User, ArrowRight, ArrowLeft } from "lucide-react";
  ...
  const filteredCabs = cabs.filter(booking => {
    ...
    if (searchQuery) {
      ...
      return (
        ...
        booking.dropoffAddress?.toLowerCase().includes(query) ||
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white tracking-wider">MY CABS</h1>
        ...
        <div className="bg-white/5 border border-white/10 mb-6 p-6">
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <Input
                  placeholder="Search by booking ref, driver, or vehicle..."
                  value={searchQuery}
        ...
        {isLoading ? (
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex-1 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-semibold text-lg text-white">{cab.vehicleType || 'Cab'}</h3>
                            <p className="text-sm text-white/50">Ref: {cab.bookingReference}</p>
                          </div>
                        </div>
                        <Badge className={`${getStatusColor(cab.status)} border`}>
                          {cab.status}
                        </Badge>
                      </div>
                      <div className="grid gap-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-white">{cab.pickupAddress || cab.fromLocation}</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <MapPin className="h-5 w-5 text-white/80 mt-0.5" />
                          <div className="flex-1">
                            ...
                            <p className="text-white">{cab.dropoffAddress || cab.toLocation}</p>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-6 text-sm text-white/60 pt-2 border-t border-white/10">
                        <div className="flex items-center gap-2">
                          ...
                          {formatDate(cab.departureDate)} at {formatTime(cab.departureDate)}
                        </div>
                        {cab.driverName && (
                          <div className="flex items-center gap-2">
                    ...
                    <div className="flex lg:flex-col items-center lg:items-end justify-between lg:justify-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="text-2xl font-bold text-white">₹{parseFloat(cab.totalAmount).toLocaleString()}</p>
                      </div>
                      <Button
                        variant="outline"
          ...
          <div className="bg-white/5 border border-white/10 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">No cabs found</h3>
