...
const savedAccounts = [
  { id: "1", name: "Home", consumerNumber: "1234567890", provider: "Indraprastha Gas", city: "Delhi", billAmount: "₹850", dueAmount: 850, dueDate: "28 Oct 2024", lastPaid: "15 Sep 2024" },
  { id: "2", name: "Parents Home", consumerNumber: "9876543210", provider: "Gujarat Gas", city: "Ahmedabad", billAmount: "₹650", dueAmount: 650, dueDate: "25 Oct 2024", lastPaid: "12 Sep 2024" },
  { id: "3", name: "Shop", consumerNumber: "5555666677", provider: "MGL", city: "Mumbai", billAmount: "₹1,200", dueAmount: 1200, dueDate: "30 Oct 2024", lastPaid: "10 Sep 2024" },
];

const allTransactions = [
  { id: "1", accountId: "1", amount: 850, date: "15 Sep 2024", status: "Success", time: "10:30 AM", billMonth: "September 2024" },
  { id: "2", accountId: "1", amount: 820, date: "10 Aug 2024", status: "Success", time: "03:15 PM", billMonth: "August 2024" },
  { id: "3", accountId: "2", amount: 650, date: "12 Sep 2024", status: "Success", time: "02:45 PM", billMonth: "September 2024" },
  { id: "4", accountId: "1", amount: 900, date: "15 Jul 2024", status: "Success", time: "11:45 AM", billMonth: "July 2024" },
  { id: "5", accountId: "3", amount: 1200, date: "10 Sep 2024", status: "Success", time: "04:20 PM", billMonth: "September 2024" },
  { id: "6", accountId: "2", amount: 680, date: "12 Aug 2024", status: "Success", time: "01:15 PM", billMonth: "August 2024" },
];

const dueBills = [
  { id: "d1", accountId: "1", amount: 850, dueDate: "28 Oct 2024", billMonth: "October 2024" },
];

const billAmounts = [
  { id: "a1", amount: 500, label: "Partial Payment" },
  { id: "a2", amount: 850, label: "Full Bill", recommended: true },
  { id: "a3", amount: 1000, label: "Extra Payment" },
  { id: "a4", amount: 1500, label: "Advance Payment" },
];

export default function GasBillAccountDetail() {
  const [, navigate] = useLocation();
  const [, params] = useRoute("/gas-bill/account/:id");
  ...
  const accountDueBills = dueBills.filter(txn => txn.accountId === params?.id);
  ...
  const handleProceedToPayment = () => {
    ...
    const paymentParams = new URLSearchParams({
      ...
      returnUrl: `/gas-bill/account/${account.id}`
    });
  ...
  const handleTransactionClick = (transaction: any) => {
    navigate(`/gas-bill/transaction/${transaction.id}`);
  };
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              onClick={() => {
                ...
                navigate(`/add-gas-account?${params.toString()}`);
              }}
              className="text-white/60 hover:text-white hover:bg-white/10 p-2 rounded-none"
      ...
      <div className="pt-20 pb-6 px-4 space-y-6 w-full max-w-screen-lg mx-auto">
        <div className="border border-white/20 bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-xl p-6">
          <div className="flex items-center gap-4 mb-6">
            <div className="w-14 h-14 border border-white/20 flex items-center justify-center">
              <Flame className="h-7 w-7 text-white/60" />
            </div>
            <div className="flex-1">
              <h2 className="text-white font-light text-xl tracking-wide mb-1">{account.name}</h2>
              <p className="text-[10px] text-white/50 uppercase tracking-widest">{account.provider} • {account.city}</p>
            </div>
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-[10px] text-white/50 uppercase tracking-widest">Consumer Number</span>
              <span className="text-white font-light">{account.consumerNumber}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[10px] text-white/50 uppercase tracking-widest">Current Bill</span>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="recent" className="mt-6 space-y-3">
            ...
            {/* Due Bills */}
            {accountDueBills.length > 0 && accountDueBills.map((bill) => (
              <div
                key={bill.id}
                className="border border-white/20 bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-xl p-4"
                data-testid={`card-due-${bill.id}`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-light tracking-wide">₹{bill.amount.toLocaleString()}</p>
                    <p className="text-[10px] text-white/50 uppercase tracking-widest mt-1">{bill.billMonth}</p>
                    <div className="flex items-center gap-1 mt-1">
                      <Calendar className="h-3 w-3 text-white/40" />
                      <p className="text-[10px] text-white/40 uppercase tracking-widest">Due: {bill.dueDate}</p>
                    </div>
                  </div>
                  <Button
                    onClick={(e) => {
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-white/50 uppercase tracking-widest mt-1">{transaction.billMonth}</p>
                    <div className="flex items-center gap-1 mt-1">
                      <Calendar className="h-3 w-3 text-white/40" />
        ...
        <DialogContent className="bg-black border-white/20 text-white max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/50 text-[10px] uppercase tracking-widest">
              Choose amount for {account.consumerNumber}
            </DialogDescription>
          </DialogHeader>

          <div className="mt-6 space-y-3">
            {billAmounts.map((option) => (
              <div
                key={option.id}
        ...
        <AlertDialogContent className="bg-black border-white/20 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/60 text-sm">
              Are you sure you want to delete "{account.name}" ({account.consumerNumber})? This action cannot be undone.
