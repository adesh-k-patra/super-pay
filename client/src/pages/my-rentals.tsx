...
import { Package, Search, Calendar, MapPin, Clock, Users, ArrowRight, ArrowLeft } from "lucide-react";
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="p-3 bg-white/10 border border-white/20 rounded-none">
              <Package className="h-8 w-8 text-white" strokeWidth={1} />
            </div>
            <div>
              <h1 className="text-3xl font-light text-white tracking-wider uppercase">My Rentals</h1>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-white/10/20 border border-pink-500/30 rounded-none">
                          <Package className="h-5 w-5 text-white/80" strokeWidth={1} />
                        </div>
                        <div>
                          <h3 className="font-light text-lg text-white">{rental.operatorName || 'Rental'}</h3>
                          <p className="text-sm text-white/50 uppercase tracking-wider">ID: {rental.bookingReference}</p>
                        </div>
                      </div>
                      <Badge className={`${getStatusColor(rental.status)} border rounded-none uppercase tracking-wider text-xs`}>
                        {rental.status}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-8">
                      <div>
                        <p className="text-2xl font-light text-white">{formatTime(rental.departureDate)}</p>
                      ...
                      <div>
                        <p className="text-2xl font-light text-white">{rental.arrivalTime || formatTime(rental.departureDate)}</p>
                        <p className="text-white/60 flex items-center gap-1 mt-1 text-sm">
                          <MapPin className="h-3 w-3" strokeWidth={1} />
                          {rental.toLocation}
                        </p>
                    ...
                    <div className="flex items-center gap-6 text-sm text-white/60">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {rental.totalPassengers} Passenger{rental.totalPassengers !== 1 ? 's' : ''}
                      </div>
                      {rental.seatClass && (
                  ...
                  <div className="flex lg:flex-col items-center lg:items-end justify-between lg:justify-center gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-light text-white">₹{parseFloat(rental.totalAmount).toLocaleString()}</p>
                    </div>
                    <Button
                      variant="outline"
        ...
        {sortedRentals.length === 0 && (
          <div className="bg-white/5 border border-white/10 rounded-none p-12 text-center">
            ...
            <h3 className="text-lg font-light text-white mb-2">No rentals found</h3>
            <p className="text-white/60">
              {searchQuery || statusFilter !== "all"
                ? "Try adjusting your search or filters"
                : "You haven't booked any rentals yet"}
            </p>
          </div>
        )}
      </div>
