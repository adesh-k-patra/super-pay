...
import { Train, Search, Calendar, MapPin, Clock, Users, ArrowRight, ArrowLeft } from "lucide-react";
...
export default function MyTrains() {
  ...
  useEffect(() => {
    if (error) {
      toast({
        ...
        description: "Failed to fetch train bookings. Please try again.",
        variant: "destructive",
      });
    }
  }, [error, toast]);
  ...
  const trains = allBookings.filter(b => b.serviceType === 'train');

  const filteredTrains = trains.filter(booking => {
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="p-3 bg-white/10 border border-white/20 rounded-none">
              <Train className="h-8 w-8 text-white" strokeWidth={1} />
            </div>
            <div>
              <h1 className="text-3xl font-light text-white tracking-wider uppercase">My Trains</h1>
              <p className="text-white/60 text-xs uppercase tracking-widest font-light">{trains.length} train{trains.length !== 1 ? 's' : ''} found</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-none mb-6 p-6">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <Input
                placeholder="Search by PNR, location, or train..."
                value={searchQuery}
          ...
          <div className="grid gap-4">
            {pagination.paginatedData.map((train) => (
              <div
                ...
                onClick={() => navigate(`/my-trips/trains/${train.id}`)}
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-white/10 border border-white/20 rounded-none">
                          <Train className="h-5 w-5 text-white/80" strokeWidth={1} />
                        </div>
                        <div>
                          <h3 className="font-light text-lg text-white">{train.operatorName || 'Train'}</h3>
                          <p className="text-sm text-white/50 uppercase tracking-wider">PNR: {train.bookingReference}</p>
                        </div>
                      </div>
                      <Badge className={`${getStatusColor(train.status)} border rounded-none uppercase tracking-wider text-xs`}>
                        {train.status}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-8">
                      <div>
                        <p className="text-2xl font-light text-white">{formatTime(train.departureDate)}</p>
                      ...
                      <div>
                        <p className="text-2xl font-light text-white">{train.arrivalTime || formatTime(train.departureDate)}</p>
                        <p className="text-white/60 flex items-center gap-1 mt-1 text-sm">
                          <MapPin className="h-3 w-3" strokeWidth={1} />
                          {train.toLocation}
                        </p>
                    ...
                    <div className="flex items-center gap-6 text-sm text-white/60">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {train.totalPassengers} Passenger{train.totalPassengers !== 1 ? 's' : ''}
                      </div>
                      {train.seatClass && (
                  ...
                  <div className="flex lg:flex-col items-center lg:items-end justify-between lg:justify-center gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-light text-white">₹{parseFloat(train.totalAmount).toLocaleString()}</p>
                    </div>
                    <Button
                      variant="outline"
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/my-trips/booking/${train.id}`);
                      }}
                      data-testid={`button-view-${train.id}`}
                    >
                      View Details
        ...
        {sortedTrains.length === 0 && (
          <div className="bg-white/5 border border-white/10 rounded-none p-12 text-center">
            <Train className="h-16 w-16 text-white/20 mx-auto mb-4" strokeWidth={1} />
            <h3 className="text-lg font-light text-white mb-2">No trains found</h3>
            <p className="text-white/60">
              {searchQuery || statusFilter !== "all"
                ? "Try adjusting your search or filters"
                : "You haven't booked any trains yet"}
            </p>
          </div>
        )}
      </div>
